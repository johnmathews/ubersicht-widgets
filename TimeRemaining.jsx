...
export const command = (dispatch) => dispatch({ type: "UPDATE", data: calculateTime() });
...
export const render = ({ daysLived, daysLeft, percentLeft }) => (
  <div>
    <Context>
      ...
      <Row>
        <Title>Lived:</Title> <Value>{daysLived}</Value>
      </Row>
      <Row>
...
const calculateTime = () => {
  ...
  const lifetimeMilliseconds = settings.lifeExpectancy * 1000 * 60 * 60 * 24 * 365;
  ...
  return {
    daysLived: `${lived.days} Days`,
    daysLeft: `${left.days} Days`,
    percentLeft,
  };
};
...
const calculateTimeRemaining = (millisecondsRemaining) => {
  ...
  const hours = formatTimePart(Math.floor((millisecondsRemaining % DAY) / HOUR));
  const minutes = formatTimePart(Math.floor((millisecondsRemaining % HOUR) / MINUTE));
  const seconds = formatTimePart(Math.floor((millisecondsRemaining % MINUTE) / MILLISECOND));
